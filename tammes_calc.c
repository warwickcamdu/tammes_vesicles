...
int main(int argc, char** argv){
    ...
    const float circle_diam = 4.0;
    const int radius_adjustment = -2;
    const int min_radius = 15;
    const int max_radius = 50;
    const int range = max_radius - min_radius + 1;

    int R;
    int result_N[range][myrepeats];
    int count = 0;
    int r = 0;
    int count_N = 130;
    int previous_N = 130;
    ...
    for (R = min_radius; R< max_radius+1; R++){
        printf("R = %d effective-R = %d \n", R, R + radius_adjustment);
        float small_r = circle_diam/(R + radius_adjustment);
        for(r=0; r<myrepeats; r++){
            printf("repeat %d\n", r);
    ...
    for (R = 0; R< range; R++){
        fprintf(fp, "%d", R+min_radius);
        for (r=0; r<myrepeats; r++){
            printf("R = %d : maximum N is %d \n", R+min_radius, result_N[R][r]);
            fprintf(fp, ",%d", result_N[R][r]);
        }
        fprintf(fp, "\n" );
    }
