...
int main(int argc, char** argv){
    ...
    int R;
    int result_N[ves_range][prot_range][myrepeats];

    for (R = 0; R< ves_range; R++){
    	for (r=0; r>prot_range; r--){
    ...
    for (R = min_ves_radius-1; R< max_ves_radius; R++){
    	for (r=min_prot_radius-1; r<max_prot_radius; r++){
    		for(i=0; i<myrepeats; i++){
    			printf("%d %d %d %d\n", R, r, i, result_N[R][r][i]);
    		}
    	}
    }
   	fclose(fp);
